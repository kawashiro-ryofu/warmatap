...
var MainManager = function() {
		...
		function i() {
			x = 1, $("#scene_loading hr")
				.css("display", "none"), $("#scene_loading hr")
				.css("width", 0), $("#scene_loading")
				.stop()
				.fadeOut(200, "linear"), p ? ($("#scene_loading")
					.stop()
					.css("display", "none"), $("#bt_back")
					.stop()
					.css("display", "none"), B && $("#bt_fs")
					.stop()
					.css("display", "none"), $("#scene_main .set")
					.stop()
					.css("display", "none")) : $("#scene_main")
				.stop()
				.fadeIn(200, "linear"), m = aidn.___waContext.currentTime, g.start(), C.start()
		}

		function n(n) {
			(U = !U) ? ($("#bt_feedback a")
				.text("反馈: 开启"), aidn.util.setCookie("fb", "on", 2592e3)) : ($("#bt_feedback a")
				.text("反馈: 关闭"), aidn.util.setCookie("fb", "off", 2592e3)), n && n.preventDefault()
		}

		function o(n) {
			(T = !T) ? ($("#bt_backtrack a")
				.text("背景音乐: 开启"), aidn.util.setCookie("bt", "on", 2592e3)) : ($("#bt_backtrack a")
				.text("背景音乐: 关闭"), aidn.util.setCookie("bt", "off", 2592e3)), n && n.preventDefault()
		}
		...
		this.init = function() {
			! function() {
				...
				.autoDensity = !0, document.getElementById("view")
					.appendChild(I.view), z = new PIXI.Container, g.init(), a(), $("#scene_top")
					.fadeIn(300), r()
			}()
		};
		...
		function v() {
			p || S || (S = !0, $("#bt_back")
				.stop()
				.fadeIn(200, "linear"), B && $("#bt_fs")
				.stop()
				.fadeIn(200, "linear"), $("#scene_main .set")
				.stop()
				.fadeIn(200, "linear"))
		}
		var p = !1;
		1 == aidn.util.getQuery()
			.auto && (p = !0), aidn.util.needExpandArea(!0);
		var B = aidn.util.enabledFullscreen();
		B && ($("#bt_fs")
				.css("display", "block"), $("#bt_fs")
				.click(function(n) {
			...
			.click(function(n) {
				$("#scene_top")
					.stop()
					.fadeOut(200, "linear"), $("#scene_loading")
					.stop()
					.fadeIn(200, "linear"), 2 == b ? i() : ((new aidn.WebAudio)
						.load(""), C.init(t, e));
				try {
					aidn.adv.hide()
				} catch (n) {}
			...
			.click(function(n) {
				$("#about")
					.stop()
					.fadeIn(200, "linear"), $("#about_cover")
					.stop()
					.fadeIn(200, "linear"), n.preventDefault()
			}), $("#bt_close,#about_cover")
			.click(function() {
				$("#about")
					.stop()
					.fadeOut(200, "linear"), $("#about_cover")
					.stop()
					.fadeOut(200, "linear")
			}), $("#bt_back")
			.click(function() {
				switch (x) {
					case 1:
						...
						g.end(), C.end(), $("#scene_top")
							.stop()
							.fadeIn(100, "linear"), $("#scene_loading")
							.stop()
							.fadeOut(100, "linear"), $("#scene_main")
							.stop()
							.fadeOut(100, "linear"), v();
						break;
					default:
						location.href = "https://github.com/kawashiro-ryofu/warmatap"
				}
			}), $("#bt_feedback a")
			.click(n), $("#bt_backtrack a")
			.click(o);
		var G, A, y = aidn.util.checkJapanese(),
			E = aidn.util.checkMobile(),
			w = "https://aidn.jp/mikutap/",
			M = "Mikutap";
		M += y ? ",初音未来10周年" : ",Miku10th", $("#bt_tw")
			.click(function(n) {
		...
		var m, I, z, b = 0,
				...
				this.init = function(n, a) {
					f = a,
						function(n) {
							...
							(r = new WebAudioManager)
							.load("data/track/track.json", a, t, i)
						}(n)
				...
				this.init = function(n, a) {
					e = a,
						function(n) {
							...
							(o = new WebAudioManager)
							.load("data/main/main.json", a, t, i)
						}(n)
				};
			...
			g = new function() {
				...
				function c(n, a, e) {
					var t, i;
					u != n && (1 != a && (u = n), u < 0 || (R.play(n % R.length, e), D = 90, S && (S = !1, $("#bt_back")
							.stop()
							.fadeOut(200, "linear"), B && $("#bt_fs")
							.stop()
							.fadeOut(200, "linear"), $("#scene_main .set")
							.stop()
							.fadeOut(200, "linear")), --x <= 0 && (i = (t = Math.floor(I.length * Math.random())) + m.length, (C[i].length ? C[i].pop() : new I[t](b, i))
							.play(), x = 12 * Math.random() + 6), t = n % m.length, (0 < C[t].length ? C[t].pop() : new m[t](b, t))
						.play()))
				}
				this.resize = function() {
					if (w) {
					...
					w = !0, b = new PIXI.Container, z.addChild(b), f = new PIXI.Container, z.addChild(f), (T = new n(b))
						.setColor(8965324, 0)
				}, this.start = function() {
					E || ($("#view")
							.on("mousedown", i), $(window)
							.on("mousemove", o), $(window)
							.on("mouseup", h), $(window)
							.on("keydown", a), $(window)
							.on("keyup", e)), (E || window.TouchEvent) && ($("#view")
							.on("touchstart", i), $(window)
							.on("touchmove", o), $(window)
							.on("touchend", h)), $("#view")
						.css("cursor", "pointer")
				}, this.end = function() {
					E || ($("#view")
							.off("mousedown", i), $(window)
							.off("mousemove", o), $(window)
							.off("mouseup", h), $(window)
							.off("keydown", a), $(window)
							.off("keyup", e)), (E || window.TouchEvent) && ($("#view")
							.off("touchstart", i), $(window)
							.off("touchmove", o), $(window)
							.off("touchend", h)), $("#view")
						.css("cursor", "auto")
						...
						this.play = function() {
							! function() {
								...
								var n, a = Math.min(G, A) * (.7 + .2 * Math.random()),
									...
									t = L();
								l.clear(), l.beginFill(t), l.drawRect(0, -e / 2, a, e), s.clear(), s.beginFill(t), s.drawRect(-e / 2, 0, e, a), l.y = 0, l.x = -a / 2, s.x = 0, s.y = -a / 2, r.rotation = 45 * Math.PI / 180, l.scale.x = 0, s.scale.y = 0, n = Math.random() < .5 ? -135 * Math.PI / 180 : 215 * Math.PI / 180, (new TimelineLite)
									.to(l.scale, .4, {
										x: 1,
										ease: Power2.easeOut
									})
					...
					I = [function(n, a) {
						...
						this.play = function() {
							! function() {
								...
								var n = X();
								c = P[n], $("#about")
									.css("background-color", "#" + c.toString(16)), Math.random() < .3 && T.flash(p), O = n, y.clear(), y.visible = !0, p = v.children.length - 1 - Math.floor(2 * Math.random()), v.setChildIndex(y, p);
								var a = Math.random() < .5,
		...
		n(), o(), aidn.util.webaudio ? ($("#ng")
			.css("display", "none"), $(".ok")
			.css("display", "block"), E && $("#scene_main .attention")
			.html("TOUCH &amp; SWIPE!"), y || $("#scene_top .attention")
			.text("* Raise the volume and enjoy!")) : ($("#ng")
			.css("display", "block"), $(".ok")
			.css("display", "none"), y || $("#ng .atten")
			.html("Sorry,<br>your device or browser doesn't support this site.")), PIXI.utils._saidHello = !0, aidn.window.resize(a)
	},
	WebAudioManager = function() {
		...
		this.load = function(n, a, e, t) {
			l = e, s = t, o = (r = a)
				.length, h.length = o, $.getJSON(n, function(n) {
					d = n, i()
				})
		}, this.play = function(n, a, e) {
